    ...
    try:
        ...
        if Strok == 2:
            ...
            send = bot.send_message(message.chat.id,
                                    "Ты допустил ошибку! Твоя запись ФИО должна выглядить так: 'Иванов Иван Иванович'")
...
def vkID(message):
    try:
        ...
        if index == 0:
            ...
            send = bot.send_message(message.chat.id,
                                    "Ты допустил ошибку! вид ствоей ссылки должен быть такой:\nhttp://vk.co/id00000000")
            bot.register_next_step_handler(send, vkID)
...
def email(message):
    try:
        ...
        if index0 > 0 or index1 > 0 or index2 > 0 or index3 > 0 or index4 > 0:
            ...
            con.commit()
            send = bot.send_message(message.chat.id,
                                    "Ваши данные успешно записаны, если необходимо, ты можешь проверить правильность их заполнения и в слушая надобности перезаписать их")
        else:
            send = bot.send_message(message.chat.id,
                                    "Вы допустили ошибку! проверьте правильность написания вашего e-mail!")
            bot.register_next_step_handler(send, email)
    except Exception:
        send = bot.send_message(message.chat.id,
                                "Вы допустили ошибку! проверьте правильность написания вашего e-mailvgvvvvvv!")
        bot.register_next_step_handler(send, email)
